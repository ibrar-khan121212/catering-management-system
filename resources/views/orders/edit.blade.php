...
<div class="max-w-xl mx-auto px-4 py-10">
    ...
    <form action="{{ route('orders.update', $order->Order_ID) }}" method="POST" class="space-y-6 bg-white p-6 rounded shadow">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="Event_ID" id="Event_ID" class="mt-1 block w-full border-gray-300 rounded-md">
                @foreach($events as $event)
                    <option value="{{ $event->Event_ID }}" {{ $order->Event_ID == $event->Event_ID ? 'selected' : '' }}>
                        {{ $event->Event_Type }}
                    </option>
                @endforeach
            </select>
