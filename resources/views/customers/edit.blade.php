        ...
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow rounded-lg p-8">
            ...
            <form method="POST" action="{{ route('customers.update', $customer->Customer_ID) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="Name" id="Name" value="{{ old('Name', $customer->Name) }}" required class="w-full border rounded p-2">
                    @error('Name') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Email" class="block text-sm font-medium">Email</label>
                    <input type="email" name="Email" id="Email" value="{{ old('Email', $customer->Email) }}" required class="w-full border rounded p-2">
                    @error('Email') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Phone" class="block text-sm font-medium">Phone</label>
                    <input type="text" name="Phone" id="Phone" value="{{ old('Phone', $customer->Phone) }}" required class="w-full border rounded p-2">
                    @error('Phone') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Address" class="block text-sm font-medium">Address</label>
                    <textarea name="Address" id="Address" rows="3" required class="w-full border rounded p-2">{{ old('Address', $customer->Address) }}</textarea>
