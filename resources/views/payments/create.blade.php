        ...
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow rounded-lg p-8">
            ...
            <form method="POST" action="{{ route('payments.store') }}" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="number" name="Customer_ID" value="{{ old('Customer_ID') }}" required class="w-full border rounded p-2">
                    @error('Customer_ID') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block font-semibold">Order ID:</label>
                    <input type="number" name="Order_ID" value="{{ old('Order_ID') }}" required class="w-full border rounded p-2">
                    @error('Order_ID') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block font-semibold">Payment Date:</label>
                    <input type="date" name="Payment_Date" value="{{ old('Payment_Date') }}" required class="w-full border rounded p-2">
                    @error('Payment_Date') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block font-semibold">Amount:</label>
                    <input type="number" step="0.01" name="Amount" value="{{ old('Amount') }}" required class="w-full border rounded p-2">
                    @error('Amount') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="block font-semibold">Method:</label>
                    <input type="text" name="Method" value="{{ old('Method') }}" required class="w-full border rounded p-2">
