...
@section('content')
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white py-10 px-4">
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow rounded-lg p-8">
            ...
            <h1 class="text-2xl font-bold mb-6">Add New Menu Item</h1>
            <form method="POST" action="{{ route('menu_items.store') }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="number" name="Price" id="Price" step="0.01" value="{{ old('Price') }}" required class="w-full border rounded p-2">
                    @error('Price') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Description" class="block text-sm font-medium">Description</label>
                    <textarea name="Description" id="Description" rows="4" required class="w-full border rounded p-2">{{ old('Description') }}</textarea>
