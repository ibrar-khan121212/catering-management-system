        ...
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow rounded-lg p-8">
            ...
            <form method="POST" action="{{ route('inventory_items.update', $item->Inventory_ID) }}" class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" name="Unit" id="Unit" value="{{ old('Unit', $item->Unit) }}" required class="w-full border rounded p-2">
                    @error('Unit') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Quantity_Available" class="block text-sm font-medium">Quantity Available</label>
                    <input type="number" name="Quantity_Available" id="Quantity_Available" value="{{ old('Quantity_Available', $item->Quantity_Available) }}" required class="w-full border rounded p-2">
                    @error('Quantity_Available') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
