...
@section('content')
    ...
    <div class="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white py-10 px-4">
        <div class="max-w-2xl mx-auto bg-white dark:bg-gray-800 shadow rounded-lg p-8">
            ...
            <form method="POST" action="{{ route('events.update', $event->Event_ID) }}" class="space-y-5">
                @csrf
                @method('PUT')
                <div>
                    <label for="Event_Type" class="block text-sm font-medium">Event Type</label>
                    <input type="text" name="Event_Type" id="Event_Type" value="{{ old('Event_Type', $event->Event_Type) }}" required class="w-full border rounded p-2">
                    @error('Event_Type') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Date" class="block text-sm font-medium">Date</label>
                    <input type="date" name="Date" id="Date" value="{{ old('Date', $event->Date) }}" required class="w-full border rounded p-2">
                    @error('Date') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Location" class="block text-sm font-medium">Location</label>
                    <input type="text" name="Location" id="Location" value="{{ old('Location', $event->Location) }}" required class="w-full border rounded p-2">
                    @error('Location') <p class="text-red-500 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label for="Client_Contact" class="block text-sm font-medium">Client Contact</label>
                    <input type="text" name="Client_Contact" id="Client_Contact" value="{{ old('Client_Contact', $event->Client_Contact) }}" required class="w-full border rounded p-2">
